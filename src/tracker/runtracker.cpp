...
void processRGBData(unsigned char* pData, int width, int height){
    ...
    // =================================== Start Tracking ===================================
    if(GLB_TrackerRes.nTrackerStatus != 3 && nFrmCnt > track_frame_cnt && IS_TRACK)
    ...
    {   
        ...
        if (select_flag == 0) // By Velocity
        {
            ...
            else 
            {
                ...
                photo_last = inputMat.clone();
                if(ok && nFrmCnt > velocity_track_frame_cnt){sorted_box = g_prev[0]; TRACK =  true;}
                else{TRACK =  false;}
            }

        }
        else if (select_flag == 1) // By XY - 从配置文件动态读取坐标
        {
            sorted_box = getBoxByXY(Coordinate_X, Coordinate_Y);
            if (sorted_box.x != -1) {
                TRACK =  true;
            }
            else {
        ...
        else if (select_flag == 2) // By ID - 从配置文件动态读取ID
        {
        ...
        if(TRACK){
            ...
            // === KCF 段错误防护 ===
            // libTrackerLib 的 KCF 在目标框周围提取约 2.5 倍的子窗口做相关滤波
            // 框太大或太靠近边缘 → 子窗口超出图像 → 段错误
            if (TRACK) {
                const float KCF_MAX_DIM = 380.0f;  // 最大框尺寸（保证 2.5x 子窗口在 1080 内）
                const float KCF_PAD = 1.25f;       // 2.5x padding 的每侧系数
                const float SAFE_BUF = 10.0f;      // 额外安全边距

                // 1) 限制框尺寸：等比缩小到安全范围
                if (ww > KCF_MAX_DIM || hh > KCF_MAX_DIM) {
                    float s = std::min(KCF_MAX_DIM / ww, KCF_MAX_DIM / hh);
                    float cx = xx + ww * 0.5f;
                    float cy = yy + hh * 0.5f;
                    ww *= s;
                    hh *= s;
                    xx = cx - ww * 0.5f;
                    yy = cy - hh * 0.5f;
                    std::cout << "[INFO] KCF safety: box scaled to "
                              << (int)ww << "x" << (int)hh << std::endl;
                }

                // 2) 确保框中心距边缘有足够空间给 KCF padding
            ...
            nTrackerCMD.nLock_x = xx; // 锁定目标的左上角坐标X
            nTrackerCMD.nLock_y = yy; // 锁定目标的左上角坐标Y
            nTrackerCMD.nLock_w = ww; // 锁定目标的宽
            nTrackerCMD.nLock_h = hh; // 锁定目标的高
            nTrackerCMD.nFrameId = nFrmCnt;
    ...
    // === Servo Vision Tracking: follow target when in TRACK state ===
    if (GLB_TrackerRes.nTrackerStatus == 3) {
        float cx = GLB_TrackerRes.xmin + GLB_TrackerRes.w / 2.0f;
        float cy = GLB_TrackerRes.ymin + GLB_TrackerRes.h / 2.0f;
        const float img_cx = width / 2.0f;
        const float img_cy = height / 2.0f;
        int16_t err_x = (int16_t)(cx - img_cx);
        int16_t err_y = (int16_t)(cy - img_cy);
        servo_vision_track(err_x, err_y, 1, 0,
                           (uint16_t)GLB_TrackerRes.w,
                           (uint16_t)GLB_TrackerRes.h);
    }

    // === Send status report to TCP client ===
    {
        status_report_t rpt;
        rpt.tracker_status = (uint8_t)GLB_TrackerRes.nTrackerStatus;
        rpt.track_x = (int16_t)GLB_TrackerRes.xmin;
        rpt.track_y = (int16_t)GLB_TrackerRes.ymin;
        rpt.track_w = (int16_t)GLB_TrackerRes.w;
        rpt.track_h = (int16_t)GLB_TrackerRes.h;
        rpt.yaw_angle = 0;
        rpt.pitch_angle = 0;
        rpt.target_count = (uint8_t)ntargetnum;
...
YoloBox getBoxByXY(float x, float y) {
    for (const auto& box : g_prev) {
        float xmax = box.x + box.w;
        float ymax = box.y + box.h;
...
static inline void drawBoxesByStatus(cv::Mat& inputMat)
{
    ...
    const cv::Scalar trackColor  = cv::Scalar(255, 255, 0);   // 青：跟踪框

    int detThick   = std::max(2, W / 640);
    int trackThick = std::max(4, W / 480);
...
static inline void ResetAllStatesOnTrackLost(int cur_status, int frame_id, bool send_unlock)
{
    ...
    if (last_status == 3 && cur_status != 3)
    {
        ...
        // Stop servo tracking when track is lost
